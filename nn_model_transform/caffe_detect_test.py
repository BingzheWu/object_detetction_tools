...
import torch
def load_image(image_file, new_size = 608):
...
def test_yolov3_detector(prototxt_file, weights_file, image_file):
    ...
    net.forward()
    print(net.blobs['layer107-yolo'].data.shape)
    return net.blobs['layer107-yolo'].data, net.blobs['layer95-yolo'].data, net.blobs['layer83-yolo'].data
def vis_detections(prototxt_file, weights_file, image_file, num_classes, masks, thresh = 0.1, anchors = [10,13,  16,30,  33,23,  30,61,  62,45,  59,119,  116,90,  156,198,  373,326]):
    ...
    for idx, feature in enumerate(features):
        ...
        stride = 608 // feature.size()[2]
        masked_anchors = [anchor/float(stride) for anchor in masked_anchors ]
